...
\[
\prob_{\Omega} \left( p^{-1} |\chi| \geq (1+\eps) |A| \right \wedge p \geq q) \leq \exp\left(-\frac{n}{12} \eps^2\right)
\]
\end{lemma}
\begin{proof}
...
{\allowdisplaybreaks
\begin{eqnarray*}
  \prob_{\Omega} \left( p^{-1} |\chi| \geq (1+\eps) |A| \right \wedge p \geq q) & \underset{t > 0}{\leq} & \prob_{\Omega} \left( \exp(t p^{-1} |\chi|) \geq \exp(t (1+\eps) |A'|) \right \wedge p \geq q) \\
...
\begin{eqnarray*}
  \prob_{\Omega'_k} ( |\chi| \geq n \wedge p=f^j) & \leq & \prob( p^{-1} |\chi| \geq f^{-j} n \wedge p \geq q) \\
    & \underset{\textrm{Eq.~\ref{eq:f_j}}}{\leq} & \prob( \tilde{p}^{-1} |\chi| \geq 2|A| \wegde p \geq q ) \\
