...
\end{isabelle_cm}
\end{algorithm}
\section{Formalization of the CVM algorithm}
...
For readers not familiar with Isabelle, the following table contains brief descriptions of the syntactic elements used in the listing: 
...
\medskip
The formalization contains four theory files.
Of course, we do not verify \cref{alg:cvm} and \cref{alg:cvm_new} separately,
as most of the proof is identical.
The entry CVM{\textunderscore}Abstract{\textunderscore}Algorithm verifies a generalized version of the CVM algorithm, wiht an abstract subsampling operation, which is required to fullfill \cref{eq:subsample_condition}.
...
\toprule
Name & Description & Lines of Code & Comments \\
\midrule
CVM{\textunderscore}Preliminary & 79 \\
CVM{\textunderscore}Abstract{\textunderscore}Algorithm & 675 \\
CVM{\textunderscore}Original{\textunderscore}Algorithm & 250 \\
CVM{\textunderscore}New{\textunderscore}Unbiased{\textunderscore}Algorithm & 139 \\
\bottomrule
\end{tabular}
