...
In this section, we present the formalization of the CVM algorithm in Isabelle/HOL using our new functional probabilistic invariant based approach.
...
The invariant-based verficiation approach is in the directory CVM{\textunderscore}Invariant from the supplementary material.
...
The specialization happens in the following theories, where CVM{\textunderscore}Original{\textunderscore}Algorithm is verficiation of the original algorithm, and CVM{\textunderscore}New{\textunderscore}Algorithm verifies the new total and unbiased variant.
It should be noted that only the last file, for the new unbiased algorithm, dependends on the new library (\cref{sec:formalization_neg_dep}) on negatively associated random variables, which is included in a separate directory, which we describe in more detail in the \cref{sec:formalization_neg_dep}.
The total number of lines required for the verification of the original algorithm is \locnew~lines.

%; we mainly discuss the~\cref{alg:cvm_new} variant from~\cref{sec:negdep} since the details are similar.
We verifed a slight generalization of~\cref{alg:cvm} where the subsampling probability can be any $f \in [\frac{1}{2};e^{-1/12}]$; the original CVM algorithm~\cite{chakraborty2022} is the special case $f=\frac{1}{2}$.

Another slight deviation is that we have better constant factors compared to the original proof by Chakraborty et al.~\cite{chakraborty2023}.
The buffer size in \cref{alg:cvm} is $12 \varepsilon^{-2} \ln(6 l \delta^{-1})$ while the buffer size used by Chakraborty et al. is $12 \varepsilon^{-2} \log_2(8 l \delta^{-1}) \approx 17.3 \varepsilon^{-2} \ln(8 l \delta^{-1})$.

This is mainly due to the fact that the invariant-based proof, does not require conditioning on particular values of $p$ --- the subsampling probability at the end of the algorithm --- in contrast the transformation-based proof applies Chernoff bounds for each possible value of $p$ (upto some lower bound $q$) and establishes the final error probability as a union bound.

However, we would like to note that, neither Chakraborty et al. seem to, nor we, have tried to optimize constants.
It would be possible to find improvements using a more careful analysis in both cases.
...
The theorem that establishes the correctness of the algorithm, i.e., that the relative error will be smaller than $\varepsilon$ with probability at least $1-\delta$ is expressed in the following snippet:
\begin{isabelle_cm}
...
Then, we introduce the abrreviation \isa{A} for the cardinality of the set of elements in the sequence \isa{xs}.
...
We could express the same arguments and same general strategy in Isabelle with little effort; we did not encounter any suprises during mechanization.
