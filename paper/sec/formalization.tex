...
In this section, we present the formalization of the CVM algorithm in Isabelle/HOL; we mainly discuss the variant from~\cref{sec:negdep} since the details are similar.
...
Nevertheless, \cref{tab:isabelle_syntax} contains brief descriptions of the syntactic elements used in the formalization.
%\medskip
\begin{table}
...
% TODO: I think we should give more of the details in the formalization, as opposed to giving a glossary of it
% For example, here are some of the intricate details I would suggest:

% 1) showing a high-level view of the way the invariant is established by induction?
% 2) describing that we actually used locales and instances to give a general shared argument for both algorithms?
% 3) we can write that we omit intricate details of the calculations, but some high-level (minor) details can be in one paragraph, e.g., any numerical analysis you needed were dealt with using existing libraries XXX and YYY (e.g., automating derivatives, approximation, etc.)?
