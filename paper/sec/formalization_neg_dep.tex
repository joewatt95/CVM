...
Let us summarize a few key formalized results for negatively associated random variables:
\begin{isabelle_cm}
...
\end{isabelle_cm}
The above is the well-known Hoeffding inequality~\cite{hoeffding1963} for negatively associated random variables.
%\begin{isabelle_cm}
%\isacommand{lemma}\isamarkupfalse%
...
\end{isabelle_cm}
It is a consequence of a more general result, which we have also shown, that permuation distributions are negatively associated.
We relied on the proof by Dubashi using the FKG inequality~\cite[Th. 10]{dubhashi1996}.%
\footnote{There is a prior proof by Joag-Dev and Proschan~\cite[Th. 2.11]{joagdev1983}, which seemed to be incorrect.}
