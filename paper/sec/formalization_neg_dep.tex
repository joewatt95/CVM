...
The formalization, which we intend to submit to the AFP as a separate entry is included in the supplementary material in the directory Neg{\textunderscore}Assoc.
It follows the definitions by Joag-Dev and Proschan~\cite{joagdev1983} closely.
...
For example the FKG~\cite[Ch. 6]{alon2000},\cite{fortuin1971} inequality
\[
...
\end{isabelle_cm}
Here, \isa{\isasymsigma} and \isa{\isasymtau} are relation directions.
It is possible to multiply relation directions, i.e., \isa{\isasymsigma\ \isacharasterisk\ \isasymtau} is the forward direction, if \isa{\isasymsigma} and \isa{\isasymtau} have the same direction, and \isa{\isasymsigma\ \isacharasterisk\ \isasymtau} is the reverse direction, otherwise.
...
The FKG inequality is a key result which enables verification of negative association of random variables.
Such as the one indicator variables for the new subsampling operation, we introduced in \cref{sec:negdep}.
