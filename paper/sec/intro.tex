...
Our mechanization is carried out in Isabelle/HOL~\cite{nipkow2002}, where we generically formalized the invariant-based argument underlying both CVM variants in 753 lines.
Specializing to the original CVM algorithm took an additional 250 lines, while the new variant took 191 more lines (with 2974 lines for the supporting negative association library).
We also mechanized the transformation-based proof by Chakraborty et al.~\cite{chakraborty2022,chakraborty2023}.
As a rough point of comparison showing the simplicity of our new proof, verification of the CVM algorithm using our new technique required only \locnew~(753 + 250)~lines, while the original proof required \locold~lines.
