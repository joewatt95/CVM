...
Yet, for \cref{alg:cvm} the random operations are far from being independent.\footnote{This was an incorrect claim in the initial published proof of CVM~\cite[Claim 6]{chakraborty2022}; a later version by the same authors~\cite{chakraborty2023} provides a correct proof.
...
The aforementioned sequence of transformations results in another randomized algorithm which can be analyzed using standard methods, from which the desired results about the original algorithm can be deduced.
Indeed, it seems impossible to analyze~\cref{alg:cvm} directly using known methods.

In this paper, we present a new technique for analyzing randomized algorithms.
The technique yields a direct and substantially more general proof of the CVM algorithm.
It is very similar to how deterministic algorithms are verified using a loop invariant.
...
The verification of the CVM algorithm using our new technique requires only 1044 lines in Isabelle~\cite{nipkow2002}, while the original proof required \todo{x} lines.
We detail some of the challenges faced when mechanizing the reduction arguments used by Chakraborty et al.~\cite{chakraborty2022,chakraborty2023} in \cref{sec:transformation_based_proof}.
...
The following section provides more background information, in particular, about the semantics of randomized algorithms.
After which, we introduce our new technique, showing how probabilistic loop invariants can be used to establish tail bounds for the original CVM algorithm.
