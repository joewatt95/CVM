...
\[
  \expect [ h ] \leq h(c)
\]
where $c$ is a non-negative real constant, $h : \Rnonneg \rightarrow \Rnonneg$ is allowed to range over a class of functions, and the expectation is taken over the distribution of the state of the algorithm after consuming each stream element.
