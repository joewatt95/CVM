...
To our knowledge, it seems impossible to analyze \cref{alg:cvm} more directly using known methods~\cite{alon2000,mitzenmacher2017,motwani1995}.
...
As a rough point of comparison, verification of the CVM algorithm using our new technique requires only 1044 lines in Isabelle/HOL~\cite{nipkow2002}, while the original proof required \todo{x} lines.

The organization of this paper is as follows.
\cref{sec:background} provides background information on randomized algorithms, in particular on their semantics in Isabelle/HOL~\cite{nipkow2002}.
...
In \cref{sec:transformation_based_proof}, we discuss some challenges faced in our alternative verification of CVM using the transformation-based proof by Chakraborty et al.~\cite{chakraborty2023}.
...
\begin{itemize}
\item formalization of the CVM algorithm, both the original version (\cref{alg:cvm}) and our new version (\cref{alg:cvm_new}) with the approach presented in \cref{sec:invariants};
