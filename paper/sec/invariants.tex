...
\begin{align*}
  ...
  \prob( I(p \geq q) p^{-1} |\chi| \geq (1+\varepsilon) |A|) \\
  & \leq \exp(-t(1+\varepsilon) |A|) M(t) \\ % = exp(-t(1+\varepsilon) |A|) \expect \left[ \prod_{s \in A} \exp( t p^{-1} X_s) \right]
  & \leq \exp(-t(1+\varepsilon)|A|) \expect \left[ I(p \geq q) \prod_{s \in A} h(p^{-1} X_s) \right] \\
  & \leq \exp(-t(1+\varepsilon)|A|) h(1)^{|A|} \leq \exp(-\frac{n}{12} \varepsilon^2)
\end{align*}
where we choose $h(x) = 1+qx (\exp(t/q)-1)$.
...
Note that $h$ is affine and it can be be easily checked%
...
We use a similar strategy, as in the original proof by Chakraborty et al.~\cite{chakraborty2022}, where they use $q = \frac{n}{4 |A|}$.
