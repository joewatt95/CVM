...
Importantly, the CVM algorithm is \emph{probably-approximately correct}, i.e., the probability that the relative error of its output estimate $R$ exceeds $\varepsilon$ is at most $\delta$, i.e.., $\prob \left( |R - |A| | > \varepsilon |A| \right) \leq \delta$ for parameters $\varepsilon$ and $\delta$. % I think here it's good to have both a text and formal description.
