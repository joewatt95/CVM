...
\author{E. Karayel\inst{1}, J. Watt\inst{2}, D. Khu\inst{2}, K. Meel\inst{3}, Y. K. Tan\inst{2}}
\institute[VFU]{
  \inst{1}%
...
\item Streaming Algorithm (one-pass)
\[
  a_1, \ldots, a_l
\]
\item Algorithm has only limited mutable state
\[
  \bigo( \varepsilon^{-2} \ln(\delta^{-1} l ) b ) 
\]
\item Approximation of the count of distinct elements
\item Randomized
\item Small error with high probability
\[
  \prob\left(|X - |A|| \geq \varepsilon\right) \leq \delta 
\]
...
\end{figure}
\end{frame}
%\section{Comparison with Previous Algorithms}
...
\hline
2001 & Bar-Yossef et al.~\cite{baryossef2002} & $\tilde{O}( \ln(\delta^{-1}) (\varepsilon^{-2}  + b ))$ & M. H. \\
2007 & HLL~\cite{flajolet2007} & $O(\ln(\delta^{-1} \varepsilon^{-2} \ln b)$ & O. M. \\
2010 & Kane et al.~\cite{kane2010} & $O( \ln(\delta^-1) (\varepsilon^{-2} + b) )$ & H. \\
2020 & B\l{}asiok~\cite{blasiok2020} & $O( \varepsilon^{-2} \ln (\delta^{-1}) + b )$ & H. E. \\
2022 & Karayel~\cite{karayel2022} & $O( \varepsilon^{-2} \ln (\delta^{-1}) + b )$ & H. E. M. \\
2023 & Chakraborty et al.~\cite{chakraborty2023} & $O( \varepsilon^{-2} \ln( \delta^{-1} l) b )$ & - \\
\end{tabular}
\end{table}
Abbreviations: 
\begin{itemize}
...
\item <3-> Discovery of a new more succint proof (1003 lines vs. 2634 lines.)\only<4->{, which is based on a \emph{new} proof technique for randomized algorithms.}
...
\only<4>{
        \fill[yellow!50,opacity=0.5] (0.07,0.846) rectangle (0.7,0.895);
...
%\end{frame}
%\begin{frame}
%\frametitle{Giry Monad II}
\end{frame}
\againframe<5>{algsplit}
\begin{frame}
\frametitle{Our algorithm in monadic notation}
\begin{eqnarray*}
  \mathrm{init} & \mbind & \mathrm{step}_1 a_1 \mbind \mathrm{step}_2 \mbind \mathrm{step}_1 a_2 \mbind \mathrm{step}_2 \cdots \\
  & \mbind & \mathrm{step_1} a_l \mbind \mathrm{step}_2 \mbind \mathrm{estimate} \\
  \mathrm{init} & = & \mathrm{return} \, (1, \emptyset)
\end{eqnarray*}
\pause
Key invariant:
\[
\expect \left[ \frac{\indicat(x \in \chi)}{p} \right]= 1
\]
for all $x$ that are present in the stream.
%\end{block}
\end{frame}
...
$\expect_m \left[ \frac{\indicat(x \in \chi)}{p} \right] = 1$
\end{block}
\begin{block}{Goal}
$ L := \expect_{m \mbind \mathrm{step}_2} \left[ \frac{\indicat(x \in \chi)}{p} \right]= 1$
\end{block}
\[
L =
\only<1>{\expect_{m \mbind \mathrm{step}_2} \left[ \frac{\indicat(x \in \chi)}{p} \right]}
\only<2>{\integral{m}
      {\left(\ift{|\chi|=n}{\integral{\mathrm{subs.}(\chi)}{\frac{\indicat(s \in \tau)}{p/2}}{\tau}}{\frac{\indicat(s \in \chi)}{p}} \right)}
      {\sigma}}
\only<3>{\integral{m}
      {\left(\ift{|\chi|=n}{\frac{p}{2}\integral{\mathrm{subs.}(\chi)}{\indicat(s \in \tau)}{\tau}}{\frac{\indicat(s \in \chi)}{p}} \right)}
      {\sigma}}
\only<4>{\integral{m}
      {\left(\ift{|\chi|=n}{\frac{p}{2}\frac{\indicat(s \in \chi)}{2}}{\frac{\indicat(s \in \chi)}{p}} \right)}
      {\sigma}}
\only<5>{\integral{m}
      {\left(\ift{|\chi|=n}{\frac{\indicat(s \in \chi)}{p}}{\frac{\indicat(s \in \chi)}{p}} \right)}
      {\sigma}}
\only<6>{\integral{m}
      {\left(\frac{\indicat(s \in \chi)}{p} \right)}
      {\sigma}}
\only<7>{1}
\]
\end{frame}

\begin{frame}<1>[label=observations]
\frametitle{Observations}
\begin{itemize}
\item Expected result is the count of distinct elements.
\only<2->{\item Can extend the above recursive analysis to more complex expressions: }
\only<3>{
\[
...
\only<5->{\item $\Rightarrow$ Approximation Guarantee
\[
  \prob\left(|X - |A|| \geq \varepsilon\right) \leq \delta 
\]
}
...
\begin{algorithmic}
  ...
  \For{$i \gets 1$ to $l$}
    \State $b \getsr \Ber(2^{-k})$ \Comment random bit $b$ from the Bernoulli distribution
    \If{$b$} \Comment insert $a_i$ if $b$ is true (with prob. $p$)
      \State $\chi \gets \chi \cup \{a_i\}$
    \Else \Comment remove $a_i$ otherwise
      \State $\chi \gets \chi - \{a_i\}$
...
Previous algorithm preserves the invariant:
\[
  s \in \chi \leftrightarrow \textrm{b[ last{\textunderscore}index}(s), 0] = \cdots = \textrm{b[ last{\textunderscore}index}(s), k-1] = 1
\]
Establish Chernoff bounds using independence of the coin flips of $b$ for all possible outcomes of $k$.
\end{frame}

\begin{frame}
\frametitle{Negative Association}
\begin{itemize}
\item Generalization of the concept of Independence
\pause
\item Obey rules similar to independent random variables
\pause
\item Ober Chernoff-Bounds similar to independent random variables
\pause
\item Formalized in a separate AFP entry: Negative{\textunderscore}Association
\pause
\item Example: The sequence of bits in an idealized Bloom Filter form an negative dependent set
\end{itemize}
