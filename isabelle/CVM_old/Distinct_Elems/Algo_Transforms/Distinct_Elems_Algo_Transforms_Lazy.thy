theory Distinct_Elems_Algo_Transforms_Lazy
...
definition step_1_lazy :: "'a list \<Rightarrow> nat \<Rightarrow> 'a state \<Rightarrow> 'a state pmf"
  where "step_1_lazy xs i state = do {
    let k = state_k state;
...
definition step_2_lazy :: "'a list \<Rightarrow> nat \<Rightarrow> 'a state \<Rightarrow> 'a state pmf"
  where "step_2_lazy xs i state = do {
    let k = state_k state;
    let chi = state_chi state;
...
definition lazy_step :: "'a list \<Rightarrow> nat \<Rightarrow> 'a state \<Rightarrow> 'a state pmf"
  where "lazy_step xs i state = step_1_lazy xs i state \<bind> step_2_lazy xs i"
...
lemma lazy_step_cong:
  ...
  unfolding lazy_step_def step_1_lazy_def step_2_lazy_def
...
next
  ...
  obtain \<omega>1 \<omega>2 where \<omega>:
    ...
    "\<omega>2 \<in> set_pmf (step_1_lazy (xs@[x]) ?n \<omega>1)"
    "\<omega> \<in> set_pmf (step_2_lazy (xs@[x]) ?n \<omega>2)"
  ...
  hence "state_k \<omega>2 \<le> length xs" using \<omega>(2) unfolding step_1_lazy_def by auto
  hence "state_k \<omega> \<le> length xs + 1" using \<omega>(3) unfolding step_2_lazy_def
...
next
  ...
  moreover have "state_chi \<omega>2 \<subseteq> insert x (state_chi \<omega>1)"
    using \<omega>(2) unfolding step_1_lazy_def
    by (auto simp add:nth_append cong:if_cong split:if_split_asm)
  ultimately have "state_chi \<omega>2 \<subseteq> set (xs@[x])" by auto
  hence "state_chi \<omega> \<subseteq> set (xs@[x])" using \<omega>(3) unfolding step_2_lazy_def
  ...
  next
    ...
    moreover have
      ...
      by (fastforce
        ...
        simp add:
          lazy_step_def step_1_lazy_def step_2_lazy_def
