...
(* lemma conj :
  ...
  using assms rel_spmf_conj_iff_ae_measure_spmf_conj by blast *)
...
(* lemma refl_eq [simp] :
  ...
  by (simp_all add: spmf_rel_eq) *)

(* lemma skip [simp] :
  \<open>\<turnstile>spmf \<lbrace>R\<rbrace> \<langle>return_spmf | return_spmf\<rangle> \<lbrace>S\<rbrace> \<equiv> (\<And> x x'. R x x' \<Longrightarrow> S x x')\<close>
  by simp *)

(* lemma skip_left [simp] :
  assumes \<open>\<And> x. lossless_spmf (f x)\<close>
  shows
    \<open>\<turnstile>spmf \<lbrace>R\<rbrace> \<langle>return_spmf | f\<rangle> \<lbrace>S\<rbrace> \<equiv> (\<And> x. \<turnstile>spmf \<lbrace>R x\<rbrace> f \<lbrace>S x\<rbrace>)\<close>
  using assms
  by simp *)
