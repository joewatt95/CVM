...
lemma neg_assoc_imp_prod_mono:
  ...
  assumes "finite I"
  assumes "neg_assoc X I"
  assumes "\<And>i. i \<in> I \<Longrightarrow> integrable M (f i \<circ> X i) \<and> mono (f i) \<and> range (f i) \<subseteq> {0..}"
...
next
  ...
  moreover have "mono (f x)" using insert(5) by simp   
  hence "mono g" using monoD unfolding g_def by (intro monoI) (auto simp:comp_def le_fun_def)
  moreover have "mono (f i)" "\<And>x. f i x \<ge> 0" if "i \<in> F" for i using that insert(5) by auto  
  hence "mono h" using monoD unfolding h_def
    by (intro monoI prod_mono conjI) (auto simp:comp_def le_fun_def ord.mono_on_def) 
  ...
  ultimately have "True = has_int_that M (\<lambda>\<omega>. g (flip X \<omega>) * h (flip X \<omega>))
    ...
    by (intro true_eq_iff neg_assoc_imp_mult_mono_nonneg[OF insert(4) 0, where \<eta>="Fwd"] 
