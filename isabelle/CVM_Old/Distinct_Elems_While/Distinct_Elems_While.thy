...
lemma diff_prob_estimate_distinct_while_no_fail_bounded_by_prob_fail :
  ...
  \<le> probprobprob_failte_distinct xs)\<close>
...
proof -
  ...
  also have \<open>\<dots> \<le> prob_fail (foldMprob_Nonep_fail_prob_failent xs initial_state_with_bad_flag)\<close>
...
lemma prob_estimate_distinct_while_is_None_or_pred_le :
  ...
  \<le> probprobprob_failte_distinct xs) +
...
proof -
  have
    ...
    by (simp
      ...
      add: prob_is_None_or_pred_eq_prob_fail_plus_prob[simplified])
